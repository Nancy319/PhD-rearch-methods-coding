...
textdata <- read.csv("data/allContentReleases.csv", encoding = "UTF-8")
textdata <- textdata %>%
  mutate(id = row_number()) %>%
  relocate(id,date=dateRelease,minister,title,content)
str(textdata)
release_corpus <- corpus(textdata$content, docnames = textdata$id, docvars = data.frame(year = substr(textdata$date, 0, 4)))
# original corpus length and its first document
ndoc(release_corpus)

corpus_sentences <- corpus_reshape(release_corpus, to = "sentences")
...
extra_stopwords <- c("nz","jacinda","ardern","announce","government's","kelvin","davis","zealand's","grant","robertson","minister","billion","million","jones","zealand","said","zealander","shane","chris","hipkins")
stopwords_extended <- c(stopwords_extended,extra_stopwords)
...
release_collocations <- textstat_collocations(corpus_tokens, min_count = 25)
release_collocations <- release_collocations[1:250, ]

corpus_tokens <- tokens_compound(corpus_tokens, release_collocations)
...
write.csv(resultGraph,"covid_coocs_release_20.csv")
