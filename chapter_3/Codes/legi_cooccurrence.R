...
textdata <- read.csv("data/legislationDoc.csv", encoding = "UTF-8")
textdata <- textdata %>%
  mutate(id = row_number()) %>%
  relocate(id,date=dateRelease,title,content,url)
str(textdata)
legiDoc_corpus <- corpus(textdata$content, docnames = textdata$id, docvars = data.frame(year = substr(textdata$date, 0, 4)))
# original corpus length and its first document
ndoc(legiDoc_corpus)

corpus_sentences <- corpus_reshape(legiDoc_corpus, to = "sentences")
...
extra_stopwords <- c("zealand","act","minister","pm","make","amendment","amend",")<U+FEFF>","section","due")
...
legiDo_collocations <- textstat_collocations(corpus_tokens, min_count = 25)
legiDo_collocations <- legiDoe_collocations[1:250, ]

corpus_tokens <- tokens_compound(corpus_tokens, legiDo_collocations)
...
write.csv(resultGraph,"covid_coocs_legiDoc_20.csv")
