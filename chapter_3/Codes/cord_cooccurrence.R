...
library(tidyr)
textdata <- read.csv("data/metadata_cord.csv", encoding = "UTF-8")
str(textdata)
textdata <- textdata %>%
  mutate(id = row_number()) %>%
  relocate(id,)
cord_corpus <- corpus(textdata$abstract, docnames = textdata$id, docvars = data.frame(year = substr(textdata$publish_time, 0, 4)))
# original corpus length and its first document
ndoc(cord_corpus)

corpus_sentences <- corpus_reshape(cord_corpus, to = "sentences")
...
cord_collocations <- textstat_collocations(corpus_tokens, min_count = 25)
hansard_collocations <- cord_collocations[1:250, ]
...
write.csv(resultGraph,"science_cooccurrence_10.csv")
